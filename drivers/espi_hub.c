...
/* Provided by power sequencing module */
...
static void host_warn_handler(uint32_t signal, uint32_t status)
{
	...
	case ESPI_VWIRE_SIGNAL_PLTRST:
		LOG_DBG("PLT_RST changed %d", status);
...
static void vwire_handler(const struct device *dev, struct espi_callback *cb,
			  struct espi_event event)
{
	LOG_DBG("VWire %d sts: %d", event.evt_details, event.evt_data);
	if (event.evt_type == ESPI_BUS_EVENT_VWIRE_RECEIVED) {
		...
		case ESPI_VWIRE_SIGNAL_SLP_S5:
			LOG_DBG("SLP %d changed %d", event.evt_details,
				event.evt_data);
			if (state_handler) {
		...
		case ESPI_VWIRE_SIGNAL_DNX_WARN:
			...
			break;
		default:
			LOG_DBG("Unhandled VWire %d", event.evt_details);
			break;
		}
	}
}
...
int espihub_init(void)
{
	...
	LOG_INF("Configuring eSPI device");
	ret = espi_config(espi_dev, &cfg);
	if (ret) {
		LOG_WRN("eSPI config returned %d (host may be absent; continue)", ret);
		/* PoC: allow boot without live host */
		ret = 0;
	}
