...
static struct task_info tasks[] = {
	...
	(defined(CONFIG_PS2_KEYBOARD) || defined(CONFIG_PS2_MOUSE) || \
	defined(CONFIG_KSCAN_EC))
	{ .thread_id = kbc_thrd_id, .can_suspend = false, .tagname = "KBC" },
	{ .thread_id = kb_thrd_id, .can_suspend = false, .tagname = "KB" },
#endif
#ifdef CONFIG_POSTCODE_MANAGEMENT
	{ .thread_id = postcode_thrd_id, .can_suspend = false, .tagname = "POST" },
#endif
	{ .thread_id = periph_thrd_id, .can_suspend = false, .tagname = "PERIPH" },
	{ .thread_id = pwrseq_thrd_id, .can_suspend = true, .tagname = "PWR" },
#ifdef CONFIG_OOBMNGR_SUPPORT
	{ .thread_id = oobmngr_thrd_id, .can_suspend = false, .tagname = "OOB" },
#endif
	{ .thread_id = smchost_thrd_id, .can_suspend = false, .tagname = "SMC" },
#ifdef CONFIG_THERMAL_MANAGEMENT
